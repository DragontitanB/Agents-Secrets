...
import { pickImage } from '../Screens/PickerImg'; // Asegúrate de que esta función esté implementada correctamente
...
export default function Home() {
    ...
    const [emergencia, setEmergencia] = useState<registros[]>([]);
    ...
    const loadData = () => {
        db.transaction((tx) => {
            tx.executeSql(
                ...
                [],
                (_, { rows }) => {
                    const resultados = rows._array as registros[];
        ...
        if (!fecha || !titulo || !descripcion || !foto || !audio) {
            ...
            try {
                await db.transaction((tx) => {
                    tx.executeSql(
                        'INSERT INTO registros (fecha, titulo, descripcion, foto, audio) VALUES (?, ?, ?, ?, ?);',
    ...
    const updateData = async () => {
        ...
        try {
            await db.transaction((tx) => {
                tx.executeSql(
                    'UPDATE registros SET fecha=?, titulo=?, descripcion=?, foto=?, audio=? WHERE id=?;',
    ...
    const handleEdit = (item: registros) => {
            ...
            <ScrollView>
                <View style={styles.formContainer}>
                    ...
                    <TouchableOpacity
                        ...
                        onPress={() => pickImage(setFoto)} // Añadido para seleccionar imagen
                    >
